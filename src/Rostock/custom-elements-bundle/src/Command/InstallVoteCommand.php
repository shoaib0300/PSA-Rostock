...
#[AsCommand(
    name: 'psa:install-vote',
    description: 'Creates the /vote page and PSA voting frontend module.',
)]
class InstallVoteCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $pageId = $this->ensurePage($io, 'vote', 'Vote', false, null);
        $moduleId = $this->ensureModule($io, 'PSA Voting', 'psa_vote', [
            'headline' => serialize(['unit' => 'h1', 'value' => 'Member voting']),
        ]);
        $this->ensureArticleWithModule($io, $pageId, 'vote', 'Vote', $moduleId);

        $io->success(sprintf(
            'Voting setup ready. Page: /vote (%d), module id: %d. Run `php vendor/bin/contao-console contao:migrate` if vote tables are missing.',
            $pageId,
            $moduleId,
        ));
    ...
    private function ensurePage(SymfonyStyle $io, string $alias, string $title, bool $protected, ?array $groupIds): int
    {
