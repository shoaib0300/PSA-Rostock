...
#[AsCommand(
    name: 'psa:install-team',
    description: 'Creates the /team page and PSA team frontend module.',
)]
class InstallTeamCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $pageId = $this->ensurePage($io, 'team', 'Team', false, null);
        $moduleId = $this->ensureModule($io, 'PSA Team', 'psa_team', [
            'headline' => serialize(['unit' => 'h1', 'value' => 'Our team']),
        ]);
        $this->ensureArticleWithModule($io, $pageId, 'team', 'Team', $moduleId);

        $io->success(sprintf(
            'Team setup ready. Page: /team (%d), module id: %d. Run `php vendor/bin/contao-console contao:migrate` if the team table is missing.',
            $pageId,
            $moduleId,
        ));
